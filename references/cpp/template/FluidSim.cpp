...
void FluidApp::solvePoisson()
{
    ...
    for (int it = 0; residual > m_tolerance && it < m_max_iter; ++it)
    {
        ...
        // std::cout << "Pressure solver: iter=" << it << ", res=" << residual << std::endl;
    }
}
...
void FluidApp::correctVelocity()
{
    ...
    for (int y = 1; y < m_res_y - 1; ++y)
        for (int x = 1; x < m_res_x; ++x)
            // TODO: update u
            u(x, y) -= m_dt * (p(x, y) - p(x - 1, y)) / m_dx;

    // Same for velocity v_{i+1/2}.
    for (int y = 1; y < m_res_y; ++y)
        for (int x = 1; x < m_res_x - 1; ++x)
            // TODO: update v
            v(x, y) -= m_dt * (p(x, y) - p(x, y - 1)) / m_dx;
}
...
void FluidApp::advectDensitySL(const Array2d &u, const Array2d &v)
{
    ...
    for (int y = 1; y < m_res_y - 1; ++y)
    {
        for (int x = 1; x < m_res_x - 1; ++x)
        {
            ...
            double last_x = x - m_dt / m_dx * last_x_velocity;
            double last_y = y - m_dt / m_dx * last_y_velocity;
