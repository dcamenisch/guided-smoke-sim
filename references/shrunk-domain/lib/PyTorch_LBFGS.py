# LBFGS code from https://github.com/hjmshi/PyTorch-LBFGS
...
class LBFGS(Optimizer):
    ...
    def _step(self, p_k, g_Ok, g_Sk=None, options={}):
        ...
        """
        options['closure'] = torch.enable_grad()(options['closure'])
        assert len(self.param_groups) == 1
            ...
            while F_new > F_k + c1*t*gtd or not is_legal(F_new):
                ...
                if(ls_step >= max_ls):
                    ...
                    if(interpolate):
                        ...
                        # store old point
                        F_prev = F_new
                        # detach from computational graph, s.t. when del F_new, the graph is deleted
